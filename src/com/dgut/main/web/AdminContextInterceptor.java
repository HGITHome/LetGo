...
//	HTTPServletResponse. SC_OK：对应的响应状态代码为200。
    ...
    public boolean preHandle(HttpServletRequest request,
        ...
        if (adminId != null) {
            // 指定管理员（开发状态）
            admin = adminMng.findById(adminId);
            if (admin == null) {
                throw new IllegalStateException("Admin ID=" + adminId
                        + " not found!");
            }
        } else {
            // 正常状态
            Integer userId = authenticationMng
                    .retrieveUserIdFromSession(session, request);
            if (userId != null) {
                admin = adminMng.findById(userId);
            }
        }
        // 此时用户可以为null
        CmsUtils.setAdmin(request, admin);
        // User加入线程变量
        AdminThreadVariable.setUser(admin);
       ...
       /* // 用户不是管理员，提示无权限。
        ...
        // 不是超级管理员，且没有访问权限，提示无权限。
        if (auth && !admin.isSuper()
                && !permistionPass(uri, admin.getRole().getPermissions(), false)) {
            request.setAttribute(MESSAGE, MessageResolver.getMessage(request,
                    "login.notPermission"));
            response.sendError(HttpServletResponse.SC_FORBIDDEN);
            return false;
    ...
    public void postHandle(HttpServletRequest request,
        ...
        if (auth && admin != null && !admin.isSuper() && mav != null
                && mav.getModelMap() != null && mav.getViewName() != null
                && !mav.getViewName().startsWith("redirect:")) {
            mav.getModelMap().addAttribute(PERMISSION_MODEL, admin.getRole().getPermissions());
        }
    }
